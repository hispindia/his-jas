...
@Controller("JASItemUnitListController")
@RequestMapping("/module/jas/itemUnitList.form")
public class ItemUnitListController {
	...
	public String delete(@RequestParam("ids") String[] ids, HttpServletRequest request) {
		...
		try {
			...
			if (ids != null && ids.length > 0) {
				for (String sId : ids) {
					JASItemUnit itemUnit = jasService.getItemUnitById(NumberUtils.toInt(sId));
					int countItem = jasService.countItem(null, itemUnit.getId(), null, null);
					if (itemUnit != null && countItem == 0) {
						jasService.deleteItemUnit(itemUnit);
					} else {
			...
			httpSession.setAttribute(WebConstants.OPENMRS_MSG_ATTR, "Can not delete itemUnit ");
			log.error(e);
		}
	...
	public String list(@RequestParam(value = "searchName", required = false) String searchName,
		...
		int total = jasService.countListItemUnit(searchName);
		String temp = "";
		...
		List<JASItemUnit> itemUnits = jasService
		        .listItemUnit(searchName, pagingUtil.getStartPos(), pagingUtil.getPageSize());
		
		model.put("itemUnits", itemUnits);
		model.put("searchName", searchName);
		model.put("pagingUtil", pagingUtil);
		
		return "/module/jas/item/itemUnitList";
	}
}
