...
<spring:message var="pageTitle" code="jas.purchaseItem.manager" scope="page"/>
...
<tr>
	...
	<tr>
		<td><spring:message code="jas.item.name"/><em>*</em></td>
		<td>
...
<table class="box">
	<tr>
		<td><spring:message code="jas.purchaseItem.quantity"/><em>*</em></td>
		<td>
...
<br/>
<input type="submit" class="ui-button ui-widget ui-state-default ui-corner-all" value="<spring:message code="jas.purchaseItem.addToSlip"/>">
<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" value="<spring:message code="jas.back"/>" onclick="ACT.go('itemPurchaseOrderForGeneralStoreList.form');">
</form>
</div>
</div>
<!-- Purchase list -->
...
<br/>
	<c:if  test="${not empty listPurchase}">
		...
		<tr>
			<td> 
				<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" value="<spring:message code="jas.purchaseItem.finish"/>" onclick="PURCHASE.processSlipItem('0');" />
				<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" value="<spring:message code="jas.purchaseItem.clear"/>"  onclick="PURCHASE.processSlipItem('1');"/>
				<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" value="<spring:message code="jas.purchaseItem.print"/>" onClick="PURCHASE.printDiv();" />
			</td>
		</tr>
		</table>
	</c:if>
...
<table border="1">
	...
	<th><spring:message code="jas.item.name"/></th>
	<th><spring:message code="jas.item.specification"/></th>
	<th><spring:message code="jas.purchaseItem.quantity"/></th>
	</tr>
	<c:choose>
	<c:when test="${not empty listPurchase}">
	<c:forEach items="${listPurchase}" var="purchase" varStatus="varStatus">
