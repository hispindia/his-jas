...
<spring:message var="pageTitle" code="jas.issueItem.manage" scope="page"/>
...
--></div>
<br/>
<input type="submit" class="ui-button ui-widget ui-state-default ui-corner-all" onclick="ISSUE.checkQtyBeforeIssue(this);" value="<spring:message code="jas.issueItem.addToSlip"/>">
<c:if  test="${empty issueItemAccount}">
<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" value="<spring:message code="jas.issueItem.createAccount"/>" onclick="ISSUE.createAccount();">
</c:if>
<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" value="<spring:message code="jas.back"/>" onclick="ACT.go('subStoreIssueItemList.form');">
</form>
</div>
</div>
<!-- Purchase list -->
...
<table class="box" width="100%" cellpadding="5" cellspacing="0">
	<tr>
	<th>#</th>
	<th><spring:message code="jas.item.subCategory"/></th>
	...
	<tr class='${varStatus.index % 2 == 0 ? "oddRow" : "evenRow" } '>
		...
		<td><a href="#" title="Remove this" onclick="JAS.removeObject('${varStatus.index}','1');">${issue.transactionDetail.item.name}</a></td>
		...
		<tr>
			<td>
				<c:if  test="${not empty listAccountDetail && not empty issueItemAccount}">
					<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all"  id="bttprocess" value="<spring:message code="jas.finish"/>" onclick="ISSUE.processSlipItem('0');" />
					<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" id="bttprint" value="<spring:message code="jas.print"/>" onClick="PURCHASE.printDiv();" />
				</c:if>
				<c:if  test="${not empty listAccountDetail || not empty issueItemAccount}">
					<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" id="bttclear" value="<spring:message code="jas.clear"/>"  onclick="ISSUE.processSlipItem('1');"/>
				</c:if>
			</td>
		</tr>
		</table>
