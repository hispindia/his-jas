...
<table class="box">
	<tr>
		...
		<th title="Unit price">Unit price</th>
		<th title="VAT">VAT</th>
	...
	<tr class='${varStatus.index % 2 == 0 ? "oddRow" : "evenRow" } '>
		...
		<td>${avaiable.unitPrice}</td>
		<td>${avaiable.VAT}</td>
		<td><em>*</em><input type="text" id="${avaiable.id }" ${!varStatus.first ? 'value=0' : ''} onchange="JAS.checkValueExt(this, '${avaiable.currentQuantity}');" name="${avaiable.id }" class="required digits" size="5"/></td>
		</tr>
	</c:forEach>
